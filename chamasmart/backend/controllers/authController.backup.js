    ...
    try {
      ...
      if (user) {
        ...
        if (!user.firebase_uid) {
          updateFields.push(`firebase_uid = $${placeholderIndex++}`);
          updateValues.push(uid);
        }

        if (!user.auth_provider || user.auth_provider === 'password') {
          updateFields.push(`auth_provider = $${placeholderIndex++}`);
        ...
        if (providedPhone && !user.phone_number) {
          updateFields.push(`phone_number = $${placeholderIndex++}`);
          updateValues.push(providedPhone);
        }
        ...
        const lastName = providedLastName || names.slice(1).join(' ') || '';
        const phoneNumber = providedPhone || firebasePhone || null;
        ...
        const newUserResult = await client.query(
          ...
          [
            ...
            providerId, picture, emailVerified || false, true,
          ],
        );
