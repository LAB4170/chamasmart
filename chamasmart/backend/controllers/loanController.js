...
const getEffectiveLoanConfig = (constitutionConfig) => {
  ...
  return {
    ...
    loan_multiplier:
      base.loan_multiplier ?? DEFAULT_LOAN_CONFIG.loan_multiplier,
    ...
    max_applications_per_day:
      base.max_applications_per_day ??
      DEFAULT_LOAN_CONFIG.max_applications_per_day,
  };
};
...
const getLoanConfig = async (req, res) => {
  try {
    ...
    if (chamaRes.rows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "Chama not found" });
    }
...
const applyForLoan = async (req, res) => {
  ...
  try {
    ...
    const maxLoan =
      totalSavings * loanConfig.loan_multiplier - outstandingLoans;
    ...
    if (requiredGuarantee > 0) {
      ...
      const guarantorIds = guarantorArray
        .map((g) => Number(g.userId))
        .filter(Boolean);
...
const repayLoan = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error recording repayment" });
  } finally {
    client.release();
  }
};
...
const exportLoansReport = async (req, res) => {
  try {
    ...
    const filename = `chama_${chamaId}_loans_report_${new Date()
      .toISOString()
      .slice(0, 10)}.csv`;
