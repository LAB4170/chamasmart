...
const createCycle = async (req, res) => {
    ...
    const { chama_id, cycle_name, contribution_amount, frequency, start_date, roster_method = 'RANDOM', manual_roster = [] } = req.body;
    ...
    try {
        ...
        const cycleResult = await client.query(
            `INSERT INTO rosca_cycles 
             ...
             RETURNING *`,
            [chama_id, cycle_name, contribution_amount, frequency, new Date(start_date)]
        );
        ...
        const rosterValues = rosterMembers.map((member, index) =>
            `(${cycle.cycle_id}, ${member.user_id}, ${index + 1}, ${contribution_amount * (members.rows.length - 1)})`
        ).join(',');
...
const getChamaCycles = async (req, res) => {
    try {
        ...
        console.error('Get chama cycles error:', error);
        res.status(500).json({
            success: false,
...
const getCycleRoster = async (req, res) => {
    try {
        ...
        console.error('Get cycle roster error:', error);
        res.status(500).json({
            success: false,
            ...
            `SELECT COUNT(*) FROM rosca_roster rr
             ...
             AND NOT EXISTS (
                 ...
                 AND c.contribution_date >= (
                     SELECT start_date FROM rosca_cycles WHERE cycle_id = $1
                 )
             )`,
            [cycleId, position]
        );
    ...
    try {
        ...
        const existingRequest = await client.query(
            `SELECT * FROM rosca_swap_requests 
             ...
             AND status = 'PENDING'`,
            [cycleId, userId, target_position, targetUser.user_id, userPosition]
        );
        ...
        await client.query(
            ...
            [cycleId, userId, target_position, reason]
        );
