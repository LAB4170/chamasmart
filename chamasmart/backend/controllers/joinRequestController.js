...
const respondToRequest = async (req, res) => {
    ...
    try {
        ...
        if (officialCheck.rows.length === 0 || !['CHAIRPERSON', 'SECRETARY', 'TREASURER'].includes(officialCheck.rows[0].role)) {
            await client.query('ROLLBACK');
       ...
       WHERE request_id = $3`,
        ...
        await createNotification({
            ...
            link: status === 'APPROVED' ? `/chamas/${joinRequest.chama_id}` : '/my-join-requests',
            relatedId: requestId
        });
