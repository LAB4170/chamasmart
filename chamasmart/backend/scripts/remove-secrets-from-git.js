...
async function main() {
  ...
  try {
    ...
    execSync(`git bundle create chamasmart-backup-${timestamp}.bundle --all`, { stdio: 'pipe' });
    console.log(`${GREEN}✅ Backup created: chamasmart-backup-${timestamp}.bundle${RESET}`);
    console.log(`   ${YELLOW}Store this file safely - it can restore your repo if something goes wrong${RESET}`);
    ...
    const pushConfirm = await question(`${BOLD}${RED}Force push to all branches? (yes/no): ${RESET}`);
    ...
    if (pushConfirm === 'yes') {
      try {
        ...
        console.log(`${YELLOW}⚠️  Force push failed. You may need to do it manually.${RESET}`);
      }
    }
