...
async function cleanupUsers() {
    try {
        ...
        try {
            const chamaCountResult = await pool.query('SELECT COUNT(*) as count FROM chamas');
        ...
        try {
            const contribCountResult = await pool.query('SELECT COUNT(*) as count FROM contributions');
        ...
        console.error('💡 Tip: Make sure database connection is working and you have proper permissions.');
        logger.error('User cleanup failed', { error: error.message, stack: error.stack });
        process.exit(1);
    } finally {
        pool.end();
    }
