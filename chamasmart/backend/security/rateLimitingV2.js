...
const otpVerifyLimiter = rateLimit({
  ...
  keyGenerator: (req) => {
    const contact = req.body?.signupToken || req.body?.phone || req.body?.email || '';
  ...
  handler: (req, res) => {
    ...
    res.status(429).json({
      success: false,
      message: 'Too many OTP attempts. Account temporarily locked. Try again after 15 minutes.',
      retryAfter: 900
    });
  }
});
...
const otpResendLimiter = rateLimit({
  ...
  message: {
    success: false,
    message: 'OTP resend rate limited. Wait at least 30 seconds before requesting again.',
    retryAfter: 30
  },
  standardHeaders: true,
  legacyHeaders: false,
...
const applyAuthRateLimiting = (req, res, next) => {
  ...
  if (method === 'POST') {
    ...
    if (path.includes('/signup/verify-otp')) return otpVerifyLimiter(req, res, next);
    if (path.includes('/signup/resend-otp')) return otpResendLimiter(req, res, next);
  }
